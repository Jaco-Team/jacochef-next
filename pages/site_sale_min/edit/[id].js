...
import { MySelect, MyTimePicker, MyTextInput, MyDatePickerNew } from "@/ui/Forms";
...
class SiteSaleMin_edit_ extends React.Component {
  ...
  generateTextDescTrue() {
    ...
    if (parseInt(promo_action) == 3) {
      ...
      textTrue =
        this.state.promo_items_sale.length - 1 == 1 ? "позицию " + itemText : "позиции " + itemText;
    }
  ...
  addItemAdd() {
    ...
    if (!check) {
      let thisItem = this.state.items.find(
        (item) => parseInt(item.id) == parseInt(this.state.addItem),
      );
  ...
  priceItemAdd() {
    ...
    if (!check) {
      let thisItem = this.state.items.find(
        (item) => parseInt(item.id) == parseInt(this.state.priceItem),
      );
...
export default function SiteSaleMin_Edit() {
  ...
  return <SiteSaleMin_edit_ promoId={router.query.id} />;
}
