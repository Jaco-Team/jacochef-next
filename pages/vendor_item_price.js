...
import { MySelect, MyTextInput, MyAutocomplite } from "@/ui/Forms";
...
class VendorItemPrice_ extends React.Component {
  ...
  async changeVendor(event, value) {
    ...
    this.setState({
      ...
      vendorCity: this.state.cities.filter(
        (city) => parseInt(city.id) == parseInt(this.state.city),
      ),
    });
  }
  ...
  update(item_id, data, event) {
    ...
    this_items.map((item, key) => {
      if (parseInt(item.item_id) == parseInt(item_id)) {
        ...
        this_items[key]["price"] = (
          parseFloat(this_items[key]["full_price"]) / parseFloat(this_items[key]["rec_pq"])
        ).toFixed(2);
      }
    });
    ...
    return (
      <>
        ...
        <Dialog
          ...
          onClose={() => {
            this.setState({ modalDialogCity: false });
          }}
          fullWidth={true}
          maxWidth={"xs"}
        >
                    ...
                    <TableRow key={key}>
                      ...
                      <TableCell>
                        <MyTextInput
                          label=""
                          value={item.comment}
                          func={this.update.bind(this, item.item_id, "comment")}
                          multiline={true}
                          maxRows={5}
                        />
                      </TableCell>
                      <TableCell>
                        <MyTextInput
                          ...
                          func={this.update.bind(this, item.item_id, "full_price")}
                        />
                      </TableCell>
                      <TableCell>
                        <MySelect
                          data={item.pqs}
                          value={item.rec_pq == 0 || item.rec_pq == "0" ? "" : item.rec_pq}
                          func={this.update.bind(this, item.item_id, "rec_pq")}
                          label=""
                        />
                      </TableCell>
                      <TableCell>
                        {item.price} / {item.ei_name}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
...
export default function VendorItemPrice() {
  return <VendorItemPrice_ />;
}
