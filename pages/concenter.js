					...
					>
						<DialogTitle style={{textAlign: 'center'}}>
							<span style={{
								...
								fontWeight: 'bold',
								marginRight: '28px'
							}}>Заказ #{this.state.showOrder.order.order_id}</span>
						</DialogTitle>
						{this.hasAccess(acces?.err_order_mod_access) ? (
							<div
								...
								>
									<ReportProblemOutlinedIcon style={{marginLeft: '10px', marginRight: '10px', color: 'red'}}/>
								...
								<Grid
									...
									{this.state.checkedDiffOrder ? (
										<div style={{display: 'flex', alignItems: 'center'}}>
											...
											/>
											<Button variant='contained' style={{marginLeft: '12px'}}>Сохранить</Button>
										</div>
									) : null}
									{
										this.state.problem_arr.length && this.state.problem_arr.find((item) => item?.problem_id === 3)?.problem_id ? (
											<div>
												...
												/>
												<Button variant='contained' style={{marginTop: '12px'}}>Сохранить</Button>
											</div>
										) : null
									}
								</Grid>
							...
							<Grid container spacing={0}>
								<Grid size={{xs: 5}}>
									<Grid
										...
										<span><b>{this.state.showOrder.order.type_order}</b> <br/> {this.state.showOrder.order.type_order_addr_new}</span>
									</Grid>
									...
									<Grid
										...
										<span><b>{this.state.showOrder.order.time_order_name}</b><br/> {this.state.showOrder.order.time_order}</span>
									</Grid>
									...
									{this.state.showOrder.order.delete_reason.length > 0 ? <Grid
										...
										size={{
											xs: 12
										}}><span style={{color: 'red'}}><b>Удален</b><br/> {this.state.showOrder.order.date_time_delete}</span></Grid> : null}
									{this.state.showOrder.order.delete_reason.length > 0 ? <Grid
										style={{marginBottom: '12px'}}
										size={{
											xs: 12
									...
									<Grid
										...
										<b>Сумма заказа </b><br/>
										<span style={{marginBottom: '12px'}}>{this.state.showOrder.order.sum_order} р</span>
									</Grid>
						...
						{parseInt(this.state.showOrder.order.is_delete) == 0 && parseInt(this.state.showOrder.order.status_order) !== 6 && parseInt(acces?.del_ord_access) ?
							<DialogActions style={{justifyContent: 'center', padding: '15px 0px', marginLeft: '40px'}}>
								<ButtonGroup disableElevation={true} disableRipple={true} variant="contained" className="BtnBorderOther" style={{marginRight: 24}}>
									<Button style={{borderRadius: '12px'}} variant="contained" className="BtnCardMain CardInCardItem" onClick={this.closeOrder.bind(this)}>Отменить заказ</Button>
								</ButtonGroup>
							</DialogActions>
							:
							null
						...
						{parseInt(this.state.showOrder.order.is_delete) == 0 && parseInt(this.state.showOrder.order.driver_id) && parseInt(acces?.withdraw_an_order_access) && parseInt(this.state.showOrder.order.status_order) !== 6 ?
						...
						{parseInt(this.state.showOrder.order.type_order_) == 1 && parseInt(this.state.showOrder.order.status_order) > 4 && parseInt(this.state.showOrder.order.check_pos) >= 0 && parseInt(acces?.client_not_access) ?
							<DialogActions style={{justifyContent: 'flex-end', padding: '15px 0px'}}>
								<ButtonGroup disableElevation={true} disableRipple={true} variant="contained" className="BtnBorderOther" style={{marginRight: 24}}>
				...
				{!this.state.showOrder ? null :
					...
					>
						...
						<DialogActions style={{paddingBottom: 24}}>
							...
							<ButtonGroup disableElevation={true} disableRipple={true} variant="contained" className="BtnBorderOther" style={{marginRight: 24}}>
								<Button variant="contained" className="BtnCardMain CardInCardItem" onClick={() => {
							...
							<Table size={'small'}>
								...
								<TableBody>
									...
									{this.state.ordersRender.map((item, key) =>
										<TableRow key={key} style={parseInt(item.is_delete) == 1 && this.hasAccess(acces?.late_access) ? {
											backgroundColor: 'red',
											color: '#fff',
											fontWeight: 'bold'
										} : {}}>
											<TableCell style={parseInt(item.dist) >= 0 ? {
												...
												cursor: 'pointer',
												fontWeight: 'inherit'
											}} onClick={this.showOrder.bind(this, item.id)}>{item.id}</TableCell>
											...
											{this.hasAccess(acces?.address_access) && <TableCell style={{
												color: 'inherit',
												fontWeight: 'inherit'
											...
											}}>{item.need_time}</TableCell>
											<TableCell style={{
												color: 'inherit',
												fontWeight: 'inherit',
												backgroundColor: this.hasAccess(acces?.late_access) ? item.cook_color : ''
											}}>{item.give_data_time == '00:00:00' ? '' : item.give_data_time}</TableCell>
											{this.hasAccess(acces?.cook_time_access) && <TableCell style={{
												color: 'inherit',
												fontWeight: 'inherit',
												backgroundColor: this.hasAccess(acces?.cook_time_access) ? item.cook_color : ''
											}}>{item.cook_time}</TableCell>}
											<TableCell style={{
												color: 'inherit',
												fontWeight: 'inherit',
												backgroundColor: this.hasAccess(acces?.late_access) ? item.all_color : ''
											}}>{item.close_order}</TableCell>
											{this.hasAccess(acces?.delay_access) &&
												<TableCell style={{color: 'inherit', fontWeight: 'inherit'}}>{item.to_time}</TableCell>}
											{this.hasAccess(acces?.dif_access) && <TableCell style={{
												color: 'inherit',
												fontWeight: 'inherit',
												backgroundColor: this.hasAccess(acces?.dif_access) ? item.all_color : ''
											}}>{item.dif}</TableCell>}
											{this.hasAccess(acces?.diff2_access) &&
												<TableCell style={{color: 'inherit', fontWeight: 'inherit'}}>{item.diff2}</TableCell>}
											{this.hasAccess(acces?.time_promise_access) && <TableCell style={{
												color: 'inherit',
												fontWeight: 'inherit'
											}}>{item.unix_time_to_client == '0' || this.hasAccess(item.is_preorder_access) == 1 ? '' : item.unix_time_to_client}</TableCell>}
											{this.hasAccess(acces?.time_dev_text_access) && <TableCell style={{
												color: 'inherit',
												fontWeight: 'inherit',
												backgroundColor: this.hasAccess(acces?.time_dev_text_access) ? item.dev_color : ''
											}}>{item.time_dev_text}</TableCell>}
