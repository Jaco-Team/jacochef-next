...
class RevizionEdit_ extends React.Component {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    const data = JSON.parse(localStorage.getItem("editRevData"));
  ...
  async getDataRev() {
    ...
    const rev_id = this.state.rev_id;
    ...
    const res = await this.getData("get_for_edit", data);

    // console.log(res);

    res.saved_data.forEach((item) => {
      res.items.forEach((it) => {
        if (it.id === item.item_id && it.type === item.type) {
          it.value = Number(it.value) + Number(item.value) * Number(item.pq);

          it.counts.forEach((count) => {
            if (it.counts.some((count) => count.need_pq === item.pq)) {
              if (count.need_pq === item.pq) {
                count.value = Number(count.value) + Number(item.value);
              }
            } else {
              it.counts.push({ need_pq: item.pq, value: item.value });
            }
          });
        }

        it.counts = it.counts.length > 1 ? it.counts.filter((count) => count.value) : it.counts;
      });
    });
    ...
    res.saved_data.forEach((item) => {
      res.pf.forEach((it) => {
        if (it.id === item.item_id && it.type === item.type) {
          it.value = Number(it.value) + Number(item.value);
        }
      });
    });
    ...
    if (revData) {
      res.items.forEach((item) => {
        ...
        item.counts =
          item.counts.length > 1 ? item.counts.filter((count) => count.value) : item.counts;
      });
  ...
  setLocalStorage(id, value, type, counts) {
    ...
    localStorage.setItem(`editRevDataPoint-${point}`, JSON.stringify(data));
  }
  ...
  getLocalStorage() {
    ...
    const revData = JSON.parse(localStorage.getItem(`editRevDataPoint-${point}`));
    ...
    if (revData?.date === formatDate(new Date())) {
      ...
      localStorage.removeItem(`editRevDataPoint-${point}`);
  ...
  async saveRev() {
    ...
    if (res.st) {
      ...
      window.location.href = "/revizion_new";
    } else {
      this.setState({
        openAlert: true,
        err_status: false,
...
export default function RevizionEdit() {
  return <RevizionEdit_ />;
}
