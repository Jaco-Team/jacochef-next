...
function FormHeader_new({ type_edit }) {
  ...
  console.log(
    "bill",
    bill?.type,
    parseInt(bill?.type) == 5 || parseInt(type) === 2 || parseInt(type) === 3,
    type,
  );
      ...
      >
        {parseInt(bill?.type) == 5 || parseInt(bill?.type) == 2 ? (
          <MyAutocomplite
            ...
            func={(event, data) =>
              changeData("pq", { target: { value: data?.name ?? event.target.value } })
            }
            onBlur={(event, data) =>
              changeData("pq", { target: { value: data?.name ?? event.target.value } })
            }
            ...
            {!imgs_bill.length ? (
              ...
              <>
                {imgs_bill.map((img, key) =>
                  img.includes(".pdf") ? (
                    <a
                      href={"https://storage.yandexcloud.net/" + url + img}
                      target="_blank"
                      rel="noreferrer"
                      key={key}
                      style={{
                    ...
                    <img
                      ...
                      onClick={() =>
                        openImageBill("https://storage.yandexcloud.net/" + url + img, "bill")
                      }
                    />
                  ),
                )}
          ...
          {!imgs_factur.length ? (
            ...
            <>
              {imgs_factur.map((img, key) =>
                img.includes(".pdf") ? (
                  <a
                    href={"https://storage.yandexcloud.net/bill/" + url + img}
                    target="_blank"
                    rel="noreferrer"
                    key={key}
                    style={{
                  ...
                  <img
                    ...
                    className="img_modal_bill"
                    onClick={() =>
                      openImageBill("https://storage.yandexcloud.net/bill/" + img, "factur")
                    }
                  />
                ),
              )}
    ...
    <Accordion>
      ...
      <AccordionDetails>
          ...
          <TableBody>
            {item?.items?.map((item, key) => (
              ...
              >
                ...
                >
                  {item.name?.color ? item.name.key : item.name}
                </TableCell>
                <TableCell
                  style={{
                    textAlign: "center",
                    background: item.pq?.color
                      ? item.pq.color === "true"
                        ? "#FFCF40"
                        : "#fadadd"
                      : null,
                ...
                >
                  {item.pq?.color ? item.pq.key : item.pq}{" "}
                  {item.ei_name?.color ? item.ei_name.key : item.ei_name}
                </TableCell>
                <TableCell
                  style={{
                    textAlign: "center",
                    background: item.count?.color
                      ? item.count.color === "true"
                        ? "#FFCF40"
                        : "#fadadd"
                      : null,
                  }}
                >
                  {item.count?.color ? item.count.key : item.count}
                </TableCell>
                <TableCell
                  style={{
                    textAlign: "center",
                    background: item.fact_count?.color
                      ? item.fact_count.color === "true"
                        ? "#FFCF40"
                        : "#fadadd"
                      : null,
                  }}
                >
                  {item.fact_count?.color ? item.fact_count.key : item.fact_count}{" "}
                  {item.ei_name?.color ? item.ei_name.key : item.ei_name}
                </TableCell>
                <TableCell
                  style={{
                    textAlign: "center",
                    background: item.price_w_nds?.color
                      ? item.price_w_nds.color === "true"
                        ? "#FFCF40"
                        : "#fadadd"
                      : null,
                  }}
                >
                  {item.price_w_nds?.color ? item.price_w_nds.key : item.price_w_nds} ₽
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow hover>
              <TableCell style={{ background: item.number?.color ? "#FFCF40" : null }}>
                {item.number?.color ? item.number.key : item.number}
              </TableCell>
              <TableCell style={{ background: date_create?.color ? "#FFCF40" : null }}>
                {date_create?.color ? date_create?.key : formatDateReverse(date_create)}
              </TableCell>
              <TableCell style={{ background: item.base_id?.color ? "#FFCF40" : null }}>
                {item.base_id?.color ? item.base_id.key : (item.base_id ?? "")}
              </TableCell>
            </TableRow>
          </TableBody>
        </Table>
