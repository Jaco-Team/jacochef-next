	...
	render() {
		...
		return (
			...
			>
									...
									<TableBody>
										{this.state.itemHist.map((it, key) => (
											<TableRow key={key} hover>
												...
												<TableCell style={{cursor: 'pointer'}} onClick={this.props.openModalHistoryView.bind(this, key, this.state.itemHist)}><TextSnippetOutlinedIcon/></TableCell>
											</TableRow>
										))}
									</TableBody>
								</Table>
				...
				<DialogActions>
					{this.props.acces?.edit_write_off_access ? <Button color="success" variant="contained" onClick={() => this.props.openItem(this.state.itemView?.id, 'Редактрование списания')}>Редактирование</Button> : null}
              ...
              <TableBody>
                ...
                <TableRow>
                  <TableCell colSpan={4}>
                    <MyTextInput
                      label="Комментарий"
                      value={this.props.itemView?.coment?.color ? this.props.itemView?.coment?.key : this.props.itemView?.coment}
                      disabled={true}
                      className={this.state.itemView?.coment?.color ? "disabled_input disabled_input_color" : "disabled_input"}
                    />
                  </TableCell>
									...
									{this.state.list.map((item, key) => (
										<TableRow hover key={key} sx={{'& td': {color: parseInt(item?.status) === 2 || item?.date_update ? '#fff' : 'rgba(0, 0, 0, 0.87)'}}} style={{backgroundColor: parseInt(item?.status) === 2 ? 'red' : item?.date_update ? 'rgb(255, 204, 0)' : '#fff'}}>
											<TableCell style={{
												cursor: 'pointer',
												fontWeight: 'bold'
											}} onClick={this.openViewItem.bind(this, item?.items, item.coment)}>{key + 1}</TableCell>
											<TableCell><Button variant="contained" onClick={this.openModalHistoryView.bind(this, item)}>{item.date}</Button></TableCell>
