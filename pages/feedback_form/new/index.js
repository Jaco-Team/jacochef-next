...
function FormBuilder() {
	...
	const updateCheckboxBlock = (index, field, value) => {
		...
		updated[index][field] = value;
		setEditingNestedElement({...editingNestedElement, data: {...editingNestedElement.data, checkboxes: updated}});
	};
			...
			case 'block':
				return (
					<Box sx={{p: 3}}>
						...
						<Typography variant="h6" sx={{mb: 2}}>Условие отображения</Typography>
						<Box sx={{mb: 3, p: 2, border: '1px solid #eee', borderRadius: 1}}>
							<Autocomplete
								...
								renderInput={(params) => (
									<TextField {...params} label="Условие отображения"/>
								)}
								sx={{mb: 2}}
							/>
							...
							{editingElement.data.condition.type === 'products' && (
								<Autocomplete
									...
									renderInput={(params) => (
										<TextField {...params} label="Выберите товары"/>
									)}
								/>
							)}
							...
							{editingElement.data.condition.type === 'orderType' && (
								<Autocomplete
									...
									renderInput={(params) => (
										<TextField {...params} label="Выберите тип заказа"/>
									)}
								/>
							)}
						</Box>

						<Typography variant="h6" sx={{mb: 2}}>Элементы в блоке</Typography>
				...
				return (
					<Box sx={{p: 3}}>
						<TextField
							...
							onChange={(e) => setEditingNestedElement({
								...element,
						...
						<Autocomplete
							...
							onChange={(_, selectedPreset) => {
								if (selectedPreset) {
									setEditingNestedElement({
										...element,
										data: {...element.data, label: selectedPreset.label, param: selectedPreset.param},
									})
						...
						<Autocomplete
							...
							onChange={(_, value) => {
								if (value.includes('Добавить новый')) {
									...
									setEditingNestedElement({
										...element,
										data: {...element.data, selectedTags: value.filter((v) => v !== 'Добавить новый')},
									});
								}
							}}
							renderInput={(params) => (
				...
				>
					<Box sx={{
						...
						<Box sx={{mt: 3, display: 'flex', justifyContent: 'flex-end'}}>
							<Button onClick={closeAddElementToBlockModal}>
								Отмена
							</Button>
						</Box>
					</Box>
