...
import { MySelect, MyCheckBox, MyAutocomplite, MyTextInput, MyDatePickerNew } from "@/ui/Forms";
...
class SiteUserManagerTable extends React.Component {
  shouldComponentUpdate(nextProps) {
    ...
    return (
      !same ||
      nextProps.page_table !== this.props.page_table ||
      nextProps.rows_table !== this.props.rows_table ||
      nextProps.total_rows !== this.props.total_rows
    );
  }

  render() {
    const { handlePageChange, handleChangeRowsPerPage, page_table, rows_table, total_rows } =
      this.props;
...
class SiteUserManager_ extends React.Component {
  ...
  sortPoint() {
    ...
    if (parseInt(city_id) == -1) {
      ...
      points_render = points.filter(
        (item) => parseInt(item.city_id) == parseInt(city_id) || parseInt(item.city_id) == -1,
      );
    }
  ...
  handleChangeRowsPerPage(event) {
    ...
    if (rows_table < 0) {
      ...
      const usersPerPage = usersCopy.slice(0 * rows_table, 0 * rows_table + rows_table);
  ...
  render() {
    return (
      <>
        ...
        >
          <DialogTitle className="button">
            <Typography>С какого периода применить изменения ?</Typography>
            <IconButton onClick={() => this.setState({ graphModal: false, graphType: 0 })}>
              <CloseIcon />
            </IconButton>
          </DialogTitle>
          <DialogContent>
        ...
        >
          <DialogTitle className="button">
            <Typography>Добавление отсутстствия сотрудника</Typography>
            <IconButton onClick={() => this.setState({ modalVacation: false })}>
              <CloseIcon />
            </IconButton>
          </DialogTitle>
                ...
                >
                  ...
                  >
                    <MyDatePickerNew
                      label="Дата начала"
                      value={
                        this.state.editUser.user.vacationStart
                          ? dayjs(this.state.editUser.user.vacationStart)
                          : null
                      }
                  ...
                  >
                    <MyDatePickerNew
                      ...
                      value={
                        this.state.editUser.user.vacationEnd
                          ? dayjs(this.state.editUser.user.vacationEnd)
                          : null
                      }
          ...
          <DialogActions>
            <Button
              onClick={this.saveVacationUser.bind(this)}
              color="primary"
            >
              Добавить
            </Button>
        ...
        >
          <DialogTitle className="button">
            <Typography>Редактирование сотрудника</Typography>
            <IconButton onClick={() => this.setState({ modalUserEdit: false, editUser: null })}>
              <CloseIcon />
            </IconButton>
          </DialogTitle>
          <DialogContent style={{ paddingBottom: 10, paddingTop: 10 }}>
                  ...
                  >
                        ...
                        {parseInt(this.state.show_access) == 0 ? (
                          ...
                          >
                            <MyCheckBox
                              ...
                              value={
                                parseInt(this.state.editUser.user.acc_to_kas) == 1 ? true : false
                              }
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <Button
                            onClick={() => this.setState({ modalVacation: true })}
                            color="primary"
                            variant="contained"
                          >
                            Отсутствие
                          ...
                          <AccordionDetails>
                            <TableContainer component={Paper}>
                              <Table size={"small"}>
                                ...
                                <TableBody>
                                  {this.state.editUser.user.vacation.map((item, key) => (
                                    <TableRow key={key}>
                                      <TableCell>{key + 1}</TableCell>
        ...
        >
          <DialogTitle className="button">
            <Typography>Новый сотрудник</Typography>
            <IconButton onClick={() => this.setState({ modalUserNew: false, editUser: null })}>
              <CloseIcon />
            </IconButton>
          </DialogTitle>
          <DialogContent style={{ paddingBottom: 10, paddingTop: 10 }}>
          ...
          <DialogActions>
            <Button
              onClick={this.saveNewUser.bind(this)}
              color="primary"
            >
              Сохранить
            </Button>
