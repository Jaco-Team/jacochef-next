...
class SitePush_Modal extends React.Component {
  ...
  save() {
    ...
    if (
      (parseInt(push.this_push.type) === 2 || parseInt(push.this_push.type) === 3) &&
      !push.this_push.it_id
    ) {
      this.setState({
        openAlert: true,
        err_status: false,
          ...
          {!this.state.push ? null : (
            <DialogContent style={{ paddingBottom: 10, paddingTop: 10 }}>
              ...
              >
                ...
                {parseInt(this.state.push.this_push.type) === 2 ||
                parseInt(this.state.push.this_push.type) === 3 ? (
                  <Grid
                    size={{
                      xs: 12,
                      sm: 6,
...
class SitePush_ extends React.Component {
  ...
  async changeCity(event) {
    ...
    if (parseInt(city) !== -1) {
      ...
      non_active = non_active.filter(
        (item) => item.city_id.includes(city || "-1") || item.city_id.includes("-1"),
      );
    }
  ...
  async openModal(mark, method, push_id) {
    ...
    if (mark === "push_new") {
      ...
      push.types = [
        { id: 1, name: "Простое уведомление" },
        { id: 2, name: "Открыть акцию" },
        { id: 3, name: "Открыть товар" },
      ];
    ...
    if (mark === "push_edit") {
      ...
      if (parseInt(push.this_push.ban_id)) {
        push.this_push.it_id =
          push.banners.find((ban) => parseInt(ban.id) === parseInt(push.this_push.ban_id)) ?? 0;
      ...
      if (parseInt(push.this_push.item_id)) {
        push.this_push.it_id =
          push.items.find((item) => parseInt(item.id) === parseInt(push.this_push.item_id)) ?? 0;
        push.this_push.list = push.items;
      }
    ...
    return (
      <>
        ...
        >
          ...
          >
            <Button
              onClick={this.openModal.bind(this, "push_new", "Новая рассылка")}
              variant="contained"
            >
              Добавить рассылку
            </Button>
