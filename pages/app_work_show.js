...
class AppWorkShow_ extends React.Component {
  ...
  async getWorks() {
    ...
    res.items.map((item, key) => {
      res.items[key]["dow_name"] = this.state.dows.find(
        ...
        >
          ...
          {!this.state.itemsEdit ? null : (
            <Dialog
              ...
              maxWidth={"md"}
              onClose={() => {
                this.setState({ modalDialog: false, itemsEdit: null, nameWork: "" });
              }}
            >
              <DialogTitle>
                Уборка "{this.state.itemsEdit.item.name}"
                <IconButton
                  onClick={() =>
                    this.setState({ modalDialog: false, itemsEdit: null, nameWork: "" })
                  }
              ...
              <DialogContent>
                ...
                >
                  ...
                  {this.state.itemsEdit.item?.show_time == false ? null : (
                    ...
                    >
                      <Typography>
                        Время за уборку: {this.state.itemsEdit.item.time_min} мин.
                      </Typography>
                    </Grid>
                  )}
                  ...
                  >
                    <Typography
                      dangerouslySetInnerHTML={{ __html: this.state.itemsEdit.item.text_work }}
                    ></Typography>
                  </Grid>
                </Grid>
              </DialogContent>
            ...
            <Table>
              ...
              <TableBody>
                {this.state.showItems.map((item, key) => (
                  <TableRow key={key}>
                    <TableCell
                      onClick={this.openWork.bind(this, item.id)}
                      style={{ color: "#c03", cursor: "pointer", fontWeight: "bold" }}
                    >
                      {item.work_name}
                    </TableCell>
                    <TableCell>
                      {item.dow_name} {item.need_work_name}
                    </TableCell>
