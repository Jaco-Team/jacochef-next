        ...
        >
          ...
          <DialogContent style={{ paddingBottom: 10, paddingTop: 10 }}>
            ...
            >
              ...
              {this.state.item &&
              parseInt(this.state.item.err.manager_create) == 1 &&
              parseInt(this.state.item.err.status) == 2 &&
              parseInt(this.state.item.err.for_dir) == 1 ? (
                <>
                  <Grid
                    size={{
                      xs: 12,
                  ...
                  >
                    <Button
                      color="primary"
                      onClick={this.saveErr_.bind(this, "true")}
                    >
                      Подтвердить ошибку
                    </Button>
                  </Grid>
                  ...
                  >
                    <Button
                      color="primary"
                      onClick={this.saveErr_.bind(this, "false")}
                    >
                      Отменить ошибку
                    </Button>
                  </Grid>
                ...
                this.state.item.user.name ? (
                  ...
                  <>
                    ...
                    >
                      {this.state.users
                        .filter((user) =>
                          this.state.app_for_search
                            ? user.app_id === this.state.app_for_search ||
                              user.new_app === this.state.app_for_search
                            : user,
                        )
                        .map((user, key) => (
                          <React.Fragment key={key}>
                            {!user.img ? null : (
                              <img
                                src={"https://storage.yandexcloud.net/user-img/max-img/" + user.img}
          ...
          {this.state.item ? (
            parseInt(this.state.item.err.for_dir) == 1 &&
            (!this.state.item.user.name || parseInt(this.state.item.err.status) == 2) ? (
              <DialogActions>
                <Button
                  color="primary"
  ...
  async save() {
    if (!this.click) {
      ...
      if (
        ...
        let text =
          "Для сохранения новой Ошибки, необходимо выбрать: Точку, Дату, Ошибку, Сотрудника и указать Время ошибки";
        ...
        if (this.myDropzone["files"].length > 0) {
          ...
          if (this.myDropzone["files"].length > 0 && this.isInit === false) {
            ...
            this.myDropzone.on("sending", (file, xhr, data) => {
              ...
              data.append(
                "filetype",
                "file_" + i + "_point_id_" + point + "_id_" + res.err_id + "." + file_type,
              );
            });
          ...
          <TableBody>
            {this.props.ret_list.map((item, i) => (
              <TableRow
                ...
                style={{
                  cursor: "pointer",
                  backgroundColor: parseInt(item.is_delete) == 1 ? "red" : "#926eae",
                  color: "#fff",
                  fontWeight: 300,
                }}
                ...
                <TableCell>
                  {!item.imgs.length
                    ...
                    : item.imgs.map((img, key) => (
                        <img
                          key={key}
                          src={"https://jacochef.ru/src/img/fine_err/uploads/" + img}
                          style={{ maxWidth: 150, maxHeight: 150, marginRight: 5 }}
                        />
                      ))}
                </TableCell>

                <TableCell style={{ textAlign: "center" }}>
                  {parseInt(item.change_win) == 1 ? <CheckIcon /> : null}
                </TableCell>
                <TableCell style={{ textAlign: "center" }}>
                  {parseInt(item.change_sum) == 1 ? <CheckIcon /> : null}
                </TableCell>
...
export default function FinesErr() {
  return <Fines_err_ />;
}
