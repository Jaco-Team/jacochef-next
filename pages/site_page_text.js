...
import { MySelect, MyTextInput, TextEditor, MyAutocomplite } from "@/ui/Forms";
...
class SitePageText_ extends React.Component {
  ...
  changeCity(event) {
    ...
    pages = pages.filter((page) =>
      parseInt(city) !== -1
        ? parseInt(page.city_id) === parseInt(city) || parseInt(page.city_id) === -1
        : page,
    );
  ...
  async openModal(mark, method, id) {
    ...
    if (mark === "editPage") {
      ...
      item.page.category_id =
        item.category.find((cat) => parseInt(cat.id) === parseInt(item.page.category_id)) ?? null;
