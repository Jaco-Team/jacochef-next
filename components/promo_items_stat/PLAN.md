...
- Активации действующими клиентами
...
```json
{
  ...
  "activations": { "min": 1, "max": 11 }
}
```

`activations` возвращает границы количества активаций промокодов в выбранном периоде.
...
- `activations`
  - optional object `{from, to}`
  - фильтрует строки `promo_table` по `total_activations`
