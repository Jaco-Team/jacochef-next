...
const buildModel = (items) => {
  ...
  const measure = (group, path = new Set()) => {
    ...
    const childGroups = children.get(group.key) || [];
...
function PositionCard({ item, disabled, activeId, scale, subordinateCount }) {
...
function GroupCard({ group, model, unitKey, disabled, activeId, scale }) {
...
function BranchNode({ group, model, unitKey, disabled, activeId, scale }) {
  const layout = model.layouts.get(group.key);
  const childGroups = model.children.get(group.key) || [];
  ...
  return (
    <Box sx={{ width: layout.width, flex: `0 0 ${layout.width}px` }}>
      ...
      <ChildDropZone
        ...
        hasChildren={Boolean(childGroups.length)}
      />
...
function DepartmentBranch({ department, disabled, activeId, scale }) {
...
export default function EmployeeAppointmentHierarchyTree({
  ...
  const handleDragEnd = ({ active, over }) => {
    ...
    onChange(items.map((item) => nextMap.get(Number(item.id)) || item));
  };
