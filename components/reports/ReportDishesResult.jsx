...
import ReportDishesColumnsDialog from "@/components/reports/ReportDishesColumnsDialog";
import ReportDishesCostDetailModal from "@/components/reports/ReportDishesCostDetailModal";
import {
  DEFAULT_REPORT_DISHES_VISIBLE_COLUMNS,
  REPORT_DISHES_COLUMNS_STORAGE_KEY,
  REPORT_DISHES_COLUMN_OPTIONS,
  REPORT_DISHES_TOGGLEABLE_COLUMNS,
} from "@/components/reports/reportDishesColumns";
...
export default function ReportDishesResult({ data, filters, onFetchCostDetail }) {
  ...
  useEffect(() => {
    ...
    try {
      const savedColumns = JSON.parse(
        window.localStorage.getItem(REPORT_DISHES_COLUMNS_STORAGE_KEY),
      );
  ...
  const saveVisibleColumns = (nextColumns) => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem(REPORT_DISHES_COLUMNS_STORAGE_KEY, JSON.stringify(nextColumns));
    }
  };

  const isColumnVisible = (key) => {
    const column = REPORT_DISHES_COLUMN_OPTIONS.find((item) => item.key === key);
  ...
  const setAllColumns = (value) => {
    const nextColumns = REPORT_DISHES_TOGGLEABLE_COLUMNS.reduce(
      (acc, item) => {
        acc[item.key] = value;
        return acc;
      },
  ...
  return (
    ...
    >
      {kpiTotals ? (
        <Grid size={{ xs: 12 }}>
          <ReportDishesKpiCards totals={kpiTotals} />
        </Grid>
      ) : null}
