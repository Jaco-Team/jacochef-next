...
class CafeEdit_ extends React.Component {
  ...
  componentDidUpdate(_, prevState) {
    if (prevState.acces !== this.state.acces) {
      // console.log("acces updated:", this.state.acces);
      this.getTabIndex();
    }
  }
  ...
  async getDataPoint() {
    ...
    res.point_info.manager_id = {
      ...
      name: res.point_info.manager_name,
    };
    const upr = res.upr_list.find(
      (upr) => parseInt(upr.id) === parseInt(res.point_info?.manager_id?.id)
    );
