...
export const ModalAddressManagementEdit = ({ open, onClose, save, addressGroup, centerMap }) => {
  ...
  useEffect(() => {
    if (open) {
      const data = addressGroup.value.map((item) => {
        return {
  ...
  const handleSave = () => {
    save(addressGroup.id, name, dataAddress);
  };
  ...
  return (
    <Dialog
      ...
      onClose={handleClose}
    >
      <DialogTitle>Редактирование группы {addressGroup.name}</DialogTitle>
      <DialogContent
        align="center"
        sx={{ fontWeight: "bold" }}
      >
