    ...
    >
      ...
      >
        <div className="lexkit-command-palette-header">
          <Search
            size={16}
            className="lexkit-command-palette-icon"
          />
          <input
            ref={inputRef}
            type="text"
        ...
        <div
          ref={listRef}
          className="lexkit-command-palette-list"
        >
          {Object.keys(groupedCommands).length === 0 ? (
            <div className="lexkit-command-palette-empty">No commands found</div>
          ) : (
            Object.entries(groupedCommands).map(([category, commands]) => (
              <div
                key={category}
                className="lexkit-command-palette-group"
              >
                <div className="lexkit-command-palette-group-title">{category}</div>
                  ...
                  return (
                    ...
                    >
                      <div className="lexkit-command-palette-item-content">
                        <div className="lexkit-command-palette-item-title">{cmd.label}</div>
                        {cmd.description && (
                      ...
                      {cmd.shortcut && (
                        <kbd className="lexkit-command-palette-item-shortcut">{cmd.shortcut}</kbd>
                      )}
                    </div>
                  );
                })}
