...
- `api/cafe_performance/metric_details`

Request body uses the same dashboard filters plus:
...
Allowed `metric` values are the clickable `get_dashboard.summary` cards: `sla_position`, `sla_order`, `p50_position`, `complaints_per_100_orders`.

Response payload fields:

- `metric.value`, `metric.label`, `metric.detail_type` - selected card metadata.
- `rows` - paginated detail groups for the selected metric.
- `pagination` - `page`, `per_page`, `total`, `total_pages`.

Detail row shapes:

- `sla_position` and `p50_position` return one row per order with nested `items`. Each item has `row_id`, `item_name`, `item_id`, `category_id`, `category_name`, `metric_stage`, `metric_stage_type`, `metric_stage_name`, `fact_seconds`, `target_seconds`, `overdue_seconds`, `sla_passed`. Parent rows include `order_id`, `point_id`, `point_name`, `items_count`, `violations_count`, `max_overdue_seconds`.
- `sla_order` returns one row per order with `order_id`, `order_date`, `point_id`, `point_name`, `order_type`, `order_type_name`, `is_preorder`, `fact_seconds`, `target_seconds`, `overdue_seconds`, `sla_passed`, overtime flags, and nested `items` for an FE submodal. The SLA basis is exactly `summary.sla_order`.
- `complaints_per_100_orders` returns one row per complained order with nested `complaints`. Each complaint has `error_id`, `reason_text`, source reason fields, `status`, `is_closed`, `has_remake_order`, `new_order_id`, `date_time_close`, and affected `items`.

Intended FE behavior: call this endpoint from dashboard metric cards instead of inflating `get_dashboard`; render by `metric.detail_type`.
