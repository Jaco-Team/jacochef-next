...
const ANOMALY_TOOLTIP_TEXT =
  "Аномалия — это этап, где затраченное время больше большего из p90 * 1.5 и sla * 1.5. Доля аномалий показывает, в какой части заказов есть такие этапы. Высокая доля аномалий может указывать на проблемы в работе кухни, доставке или других этапах процесса.";
...
function QualitySummaryCard({ label, value, caption, tone = "default", icon, delta, tooltipText }) {
...
export default function QualityTab({ data, formatters }) {
  ...
  const [selectedStageType, setSelectedStageType] = useState(
    filters?.stage_type || stageTypes?.[0]?.id || "",
  );

  const filteredAnomalies = useMemo(
    () => anomalies.filter((item) => !selectedStageType || item?.stage_type === selectedStageType),
    [anomalies, selectedStageType],
  );

  const sortedAnomalies = useMemo(() => {
    return [...filteredAnomalies].sort((left, right) => {
      const result = compareValues(left?.[sortBy], right?.[sortBy]);
      return sortDirection === "asc" ? result : -result;
    });
  }, [filteredAnomalies, sortBy, sortDirection]);

  const maxAnomalyShare = useMemo(
    () =>
      filteredAnomalies.reduce((max, item) => {
  ...
  const handleSort = (field) => {
    ...
    setSortBy(field);
    setSortDirection(field === "category_name" ? "asc" : "desc");
  };
  ...
  const stageSwitch = (
    <ToggleButtonGroup
      ...
      onChange={(_, value) => {
        if (value) setSelectedStageType(value);
      }}
      sx={{
      ...
      >
        ...
        >
          <QualitySummaryCard
            ...
            tooltipText={ANOMALY_TOOLTIP_TEXT}
            delta={buildDelta({
