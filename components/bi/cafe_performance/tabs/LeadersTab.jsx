...
export default function LeadersTab({ data, formatters, filters, onEmployeeOpen }) {
  ...
  const rankingWithRating = useMemo(
    () =>
  ...
  const sortedRanking = useMemo(() => {
    return [...rankingWithRating].sort((left, right) => {
      const leftValue = left?.[sortBy];
      const rightValue = right?.[sortBy];
      const result = compareValues(leftValue, rightValue);
      return sortDirection === "asc" ? result : -result;
    });
  }, [rankingWithRating, sortBy, sortDirection]);
  ...
  const handleSort = (field) => {
    ...
    setSortBy(field);
    setSortDirection(field === "point_name" || field === "rating" ? "asc" : "desc");
  };
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell
                    sx={{ width: 90 }}
                    sortDirection={sortBy === "rating" ? sortDirection : false}
                  >
                    <TableSortLabel
              ...
              <TableBody>
                {sortedRanking.map((item, index) => (
                  ...
                  >
                    <TableCell>
                      <RankBadge position={item.rating} />
                    </TableCell>
                    <TableCell>
                      <Stack
                        direction="row"
