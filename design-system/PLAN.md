...
## Зафиксированная стратегия внедрения

- Дизайн-система вводится как **forward-only default path**, а не как кампания по немедленной замене всего legacy-кода.
- Любой новый модуль и любой новый reusable UI обязан начинаться с `design-system/shared/ui`, токенов и существующей Storybook story. Новый import из `ui/*` или `ui/My*` не допускается.
- Если нужного паттерна в DS нет, сначала создается автономный `Jaco*` компонент, его colocated story с normal/variant/state сценариями, затем он используется в feature-коде.
- Legacy `ui/*` остается совместимым и поддерживаемым для существующих модулей. Его не удаляют и не переподключают массово; точечная замена разрешена только как явная часть отдельной задачи или при доработке конкретного модуля.
- Storybook является внутренним контрактом нового UI: Figma задает внешний референс, а Storybook фиксирует реализованные tokens, состояния и поведение.
...
- При новой разработке и при явно scoped migration заменять локальные magic values на токены только в task-affected файлах.
...
Первая реализация не должна переписывать весь интерфейс. Безопасный порядок:
...
6. После этого все новые модули строить через DS; существующие модули переводить только по отдельному scope.
...
- Для каждого нового модуля или модуля, явно переведенного на дизайн-систему, добавлять минимум один стабильный desktop visual smoke/baseline через Playwright.
...
- [ ] Поддерживать DS как обязательный путь для новых модулей без глобального rewire legacy-кода.
- [ ] Добавлять недостающие `Jaco*` паттерны story-first до первого использования в новом feature-коде.
- [ ] Добавлять visual baseline для нового DS-модуля только при отдельном module scope.
