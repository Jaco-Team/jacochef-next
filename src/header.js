...
export default function Header() {
  ...
  return (
    <>
      ...
      >
        <div
          style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", padding: 10 }}
        >
          <Autocomplete
            size="small"
            options={FullMenu}
            ...
            filterOptions={(options, { inputValue }) => {
              ...
              return options.filter((option) => {
                ...
                return (
                  option.navs_id &&
                  option.navs_id.some((nav) => nav.name.toLowerCase().includes(searchTerm))
                );
              });
            }}
            ...
            renderOption={(props, option, { inputValue, selectedOptions }) => {
              ...
              const matchingNavs = option.navs_id
                ? option.navs_id.filter((nav) => nav.name.toLowerCase().includes(searchTerm))
                : [];
            ...
            getOptionKey={(option) =>
              `${option.name}_${option.id}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
            }
            renderInput={(params) => (
              <TextField
                {...params}
              ...
              <List style={{ width: "100%" }}>
                {item.chaild.map((it, k) => (
                  <Link
                    ...
                    onClick={closeMenu}
                  >
                    <ListItemButton style={{ height: 50 }}>{it.name}</ListItemButton>
                  </Link>
                ))}
              </List>
            </AccordionDetails>
